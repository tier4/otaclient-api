...
class CustomBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        for config in self.config["proto_builds"]:
            ...
            proto_file_abs = str(root / proto_file)
            ...
            if not os.path.exists(proto_file_abs):
                raise FileNotFoundError(f"{proto_file_abs=} not found, abort")

            # use the project root as work_dir
            work_dir = str(root)
